...
def lstm():
	...
	print('Loading data...') 
	X_train, X_validation, X_test, Train_mask, Val_mask, Test_mask = open_mfcc_file('noisy_mfcc_1.txt',431,145,145)
	Y_train, Y_validation, Y_test = open_vad_file('noisy_mfcc_1.txt','vad_1.txt',431,145,145)

	print('Build model...')
	model = Sequential()
	model.add(LSTM(128, dropout=0, recurrent_dropout=0, input_shape=(None,20), return_sequences=True))
	model.add(LSTM(128, dropout=0, recurrent_dropout=0, input_shape=(None,20), return_sequences=True))
...
def open_mfcc_file(file, train_size, validation_size, test_size):
	...
	train_mask = np.zeros((train_size, train_max_frame), dtype='float32')
	val_mask = np.zeros((validation_size, validation_max_frame), dtype='float32')
	test_mask = np.zeros((test_size, test_max_frame), dtype='float32')
...
if __name__ == '__main__':
	# vad_order('noisy_mfcc_1.txt','vad_1.txt')
