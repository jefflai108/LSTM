...
def lstm():
	...
	print('Loading data...') 
	X_train, X_validation, X_test, Train_mask, Val_mask, Test_mask = open_mfcc_file('noisy_mfcc_1.txt',504,72,145)
	Y_train, Y_validation, Y_test = open_vad_file('noisy_mfcc_1.txt','vad_1.txt',504,72,145)

	print('Build model...')
	model = Sequential()
	model.add(TimeDistributed(Dense(64, activation='relu'), input_shape=(None,20)))
	model.add(LSTM(64, dropout=0.2, recurrent_dropout=0.2, input_shape=(None,20), return_sequences=True))
	model.add(LSTM(64, dropout=0.2, recurrent_dropout=0.2, input_shape=(None,20), return_sequences=True))
